				...
				//thread::spawn(move || {
					...
					glib::timeout_add_seconds(1, ||  {
						if seconds > 0.0 {
							...
							let minutes = (seconds as i32 % 3600) / 60;
							//println!("O valor de minutes no receiver é: {}", minutes);
							let seconds = (seconds as i32 % 3600) % 60;
